...
function patch(){if(patched)return;patched=true;function patchedExpress(...args){const app=originalExpress(...args);const use=app.use.bind(app),listen=app.listen.bind(app);let injected=false;app.use=function(...args){if(!injected){injected=true;use(marketMiddleware)}return use(...args)};app.listen=function(...a){const server=listen(...a);attach(server);return server};return app}Object.assign(patchedExpress,originalExpress);patchedExpress.static=originalExpress.static;const p=require.resolve("express");if(require.cache[p])require.cache[p].exports=patchedExpress}
patch();
